...
class LookupSplitter(master: ActorRef, originalNode: Contact, routingTable: ActorRef)(implicit config: KadActConfig) extends Actor
                                                                                                                             with FSM[LookupSplitter.State, Unit]
                                                                                                                             with LoggingFSM[LookupSplitter.State, Unit] {
  import LookupSplitter._

  val nodeLookup = context.actorOf(Props(new NodeLookup(originalNode, routingTable)), "NodeLookup")
